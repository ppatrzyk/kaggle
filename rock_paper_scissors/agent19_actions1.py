...
def reactive_agent(my_actions, enemy_actions):
    ...
    try:
        last = (my_actions[-1], enemy_actions[-1])
        reactions = list(zip(
            tuple(zip(my_actions[:-1], enemy_actions[:-1])),
            enemy_actions[1:]
        ))
        reactions = reactions[-MEMORY:]
        counter = Counter(reactions)
        counter = {key[1]: val for (key, val) in counter.items() if key[0] == last}
        sys.stdout.write(f'Resp to {last}: {str(counter)}')
