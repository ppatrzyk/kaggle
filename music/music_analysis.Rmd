...
Let's investigate and quantify the problem. First these are top 10 repeated artist names in the dataset (exact name match, case insensitive):
...
```{r conflictedcountriesexample, echo=FALSE}
lfm[
...
There are `r tags_lfm[, .N])` unique tags in the dataset. `r round(100 * (tags_lfm[N == 1, .N] / tags_lfm[, .N]), 2)`% of them are used only once and only `r tags_lfm[N >= 10, .N], ' (', round(100 * (tags_lfm[N >= 10, .N] / tags_lfm[, .N]), 2)`% of them are used at least 10 times. Worcloud of the most common tags is depicted below:
...
- if the picked tag is not related to genre, iteratively replace by next tag in the rank,
...
filter_pattern <- paste(
  ...
  "singer", "songwriter", "soundtrack", "beautiful", "awesome", "scandinav", "streamable", "duo", "radio", "century", '^african*$', 'asian*$', '^cute$', "photographer", "pianist", "francophone", 'français',
...
This approach yields `r lfm[, uniqueN(genre)]` unique genres, which are now aggregated. The approach here is to either find all instances of given genre via regex and unify all to one spelling (e.g., *hip-hop*), or if there are a lot of subtypes of given genre, retain some arbitrary number of most frequest ones and aggregate remainig ones into a general type (e.g., *metal*). Less frequent tags are aggregater as *other*. As I wanted to keep the number of unique genres minimal so there are some necessary arbitrary simplifications in this step (e.g., *brutal apocalyptic black gothic doom metal* becomes *black metal*).
...
lfm[grepl("r *n *b( |$)|r&b|rhyth?m.*&.*blues|rhyth?m and blues", genre), genre := 'rnb']
...
Now let's see how popularity is distrubuted in the entire dataset.
...
print(summary(lfm[, listeners_lastfm]))
print(ecdf_plot)
...
- in Empirical CDF plot, x axis is pruned at 50,000 listeners -- this is approximately `r paste(round(100 * 50000 / lfm[1L, listeners_lastfm], 2), "%")` of how many listeners the top artist in dataset (i.e., Coldplay) has. 
- Histogram displays count on a 4th-root-transformed scale.
...
scrobbles[, total := sum(scrobbles_lastfm)]
...
- 50% of the world's scrobbles is generated by `r sprintf('%s (%s%%)', scrobbles[, which.min(distance_half)], round(100 * scrobbles[, which.min(distance_half)] / scrobbles[, .N], 2))`
- 90% of all scrobbles are generated by `r sprintf('%s (%s%%)', scrobbles[, which.min(distance_90)], round(100 * scrobbles[, which.min(distance_90)] / scrobbles[, .N], 2))`
...
genres <- lfm[
  ...
  .(
    ...
    allscrobbles = sum(scrobbles_lastfm),
    max_listeners = listeners_lastfm[1],
    median_listeners = median(listeners_lastfm),
    median_scrobbles = median(scrobbles_lastfm)
  ),
  by = genre
]

# genres_top <- melt.data.table(
#   genres[
...
#     order(genre),
#   ],
#   measure.vars = c('artists', 'allscrobbles')
# )
# 
# genres_bar <- ggplot(genres_top, aes(genre, value, fill = variable)) +
...
genres_scatter <- ggplot(genres_top, aes(artists, allscrobbles)) +
  geom_point(fill = 'orange', color = 'black', stroke = 1, size = 4, alpha = 0.4) +
  scale_x_continuous(labels = display_values) + 
  scale_y_continuous(labels = display_values) +
  labs(
    title = "Artists and total scrobbles by genre",
    x = "Number of artists",
    y = "Aggregated scrobbles"
  ) + 
  annotate(
    'text',
    x = genres_top$artists,
    y = genres_top$allscrobbles,
    label = genres_top$genre,
    size = 3.5
  )

#TODO log scale
#heatmap country v genre
# withincountry popularity: maps for chosen genres (% of country-level production)
...
First, let's examine the ranking of all counties by the total number of scrobbles their artists produce. Countries on map are colored by the total number of scrobbles on a log scale and labeled by the top artists coming from there.

```{r topbycountry, echo=FALSE}
world_scrobbles <- lfm[, sum(scrobbles_lastfm)]
country_all <- lfm[,
  .(
    artists = .N,
    topartist = artist[1],
...
country_all[, percent_of_all_scrobbles := round(100 * allscrobbles / world_scrobbles, 2)]
rm(world_scrobbles)
...
datatable(country_all)

country_all[, logscrobbles := log10(allscrobbles + 1)]

topartist_data <- world
topartist_data@data <- merge(
  topartist_data@data,
  country_all[,
    .(country, topartist, logscrobbles, allscrobbles, popup = paste(display_values(logscrobbles)))
  ],
  all.x = TRUE,
  by = 'country'
)
...
color_pal <- colorNumeric(
  palette = "plasma",
  reverse = TRUE,
  domain = topartist_data@data$logscrobbles, 
...
totalscrobbles_map <- addPolygons(
  ...
  smoothFactor = 0.2, fillOpacity = 0.6, fillColor = ~color_pal(logscrobbles),
  popup = topartist_data@data$popup,
  options = pathOptions(pane = "polygons")
) %>%
  leaflet::addLegend(
    position = "bottomright", values = topartist_data@data$logscrobbles,
    title = "Genre", pal = color_pal, opacity = 1
  ) %>%
  addLabelOnlyMarkers(
...
totalscrobbles_map
# TODO: fix legend (log conversion)
```

When looking at raw data, almost half (`r country_all[country == 'United States', percent_of_all_scrobbles]`%) of the songs listened to are made by artists coming from the United States. The next country, United Kingdom, generates `r country_all[country == 'United Kingdom', percent_of_all_scrobbles]`% of all scrobbles. This skew is even more apparent when looking at the origin of the top 1000 artists in the world:

```{r top100, echo=TRUE}
datatable(
  lfm[
      1:1000, 
      .(artists_in_top1k = .N, top = artist[1]), 
      by = country
    ][
      order(artists_in_top1k, decreasing = TRUE), 
    ]
)
```

Only `r lfm[1:1000, ][!(country %in% c('United States', 'United Kingdom')), .N]` of the top 1k artists do not come from either US or UK. In addition, the remainder either comes from other English-speaking countries or performs in English anyway. The only exceptions to the dominance of english music in *the mainstream* are all classical artists and German band *Rammstein*.

Having confirmed and quantified what is widely knows about the mainstream music, let's investigate whether this dominance is present for all genres.

For which genres top artist is not from USA or UK? Among most common 100 genres:

```{r topnotus, echo=FALSE}
datatable(
  genres[
    order(artists, decreasing = TRUE), 
  ][
    1:100, 
  ][
    !(topartist_country %in% c('United States', 'United Kingdom')),
    .(genre, topartist_country, topartist)
  ]
)
```

Second, let't investigate the origin of scrobbles on a genre level. Here I isolate top genres and present proportions of all scrobbles coming from bands coming from different countries.

```{r genreheatmap, echo = TRUE}
# TODO/
```
...
```{r Metal, echo=FALSE}
top20_countries <- lfm[grepl('metal', genre), .N, by = country][order(-N)][1:20, ]
metal <- lfm[
  grepl('metal', genre), 
  .(
    allscrobbles = sum(scrobbles_lastfm),
...
relative popularity within a country
