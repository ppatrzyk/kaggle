...
For instance there are `r tags_lfm[grepl('black.*metal', V1), .N]` unique *black metal* tags. Top ten:

```{r blackmetalexample, echo=FALSE}
head(tags_lfm[grepl('black.*metal', V1), ], 10)
#TODO datatable(tags_lfm[grepl('black.*metal', V1), ])
```
...
for (i in 1:10) {
  ...
  # print(paste('run', i, "set:", lfm[genreset == TRUE, .N]))
}
...
Ranking of al artists:

```{r toptenartist, echo=TRUE}
# head(lfm, 10)
datatable(lfm[1:1000, ])
```
...
- half (50%) of the world's scrobbles is generated by `r sprintf('%s (%s%%)', scrobbles[, which.min(distance_half)], round(100 * scrobbles[, which.min(distance_half)] / scrobbles[, .N], 2))`
...
```{r topgenres, echo=TRUE, warning=FALSE}
genres <- lfm[
  !is.na(genre),
  .(
    artists = .N,
...
world <- geojson_read("https://raw.githubusercontent.com/nvkelso/natural-earth-vector/master/geojson/ne_110m_admin_0_countries.geojson", what = "sp")
world@data <- merge(
  world@data[, c('POP_EST', "ISO_A3")], 
  countries, 
  by.x = "ISO_A3",
  by.y = "A3",
  all.x = TRUE
)
world@data <- join(world@data, country_data, by = "A3", type = "left")
...
topartist_data@data <- merge(
  topartist_data@data,
  country_all[,
    .(country, tag_display = topartist_genre, popup = topartist_genre)
  ],
  all.x = TRUE,
  by = 'country'
)
...
color_pal <- colorFactor(
  palette = "Dark2",
  domain = topartist_data@data$tag_display,
  na.color = "transparent"
)

# TODO fix number of tgas to be displayed
# TODO add country labels

topartist_map <- addPolygons(
  world_map, data = topartist_data,
  color = "black", opacity = 1, weight = 1,
  smoothFactor = 0.2, fillOpacity = 0.6, fillColor = ~color_pal(tag_display),
  popup = topartist_data@data$popup,
  options = pathOptions(pane = "polygons")
) %>%
  leaflet::addLegend(
    position = "bottomright", values = topartist_data@data$tag_display,
    title = "Genre", pal = color_pal, opacity = 1
  )
```

TODO: how many american/uk music, which genres have top artist that are outside
