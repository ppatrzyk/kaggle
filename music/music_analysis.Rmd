...
In the next step I assign genre to each artist. This step requires investigating all tags and identifying unique genres in them. This requires some aggregation over different spelling, qualifiers, etc. First I read all tags to a separate data.table. Most frequent last.fm tags:
...
tags_lfm[, V1 := factor(V1, levels = rev(V1))]
...
tags_bar <- ggplot(
  ...
  coord_flip() +
  labs(
    title = "Most common tags on Last.fm",
    x = "Tag",
    y = "Times used"
  ) +
  theme_bw()
...
**TODO: some plot here. rectangle with proportions**
...
Only `r lfm[1:1000, ][!(country %in% c('United States', 'United Kingdom')), .N]` of the top 1k artists do not come from either US or UK. However, the remainder either comes from other English-speaking languages or performs in English anyway. **TODO possibly move**: The only exceptions to the dominance of english music are all classical artists (more on that below) and German band *Rammstein*.
