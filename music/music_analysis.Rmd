...
For instance there are `r tags_lfm[grepl('black.*metal', V1), .N]` unique *black metal* tags. Top 100:

```{r blackmetalexample, echo=FALSE}
datatable(tags_lfm[grepl('black.*metal', V1), .(tag = V1, count = N)][1:100, ])
```
...
As all would expect, popularity is extremely skewed -- the vast majority of artists have a very small number of listeners and scrobbles while there are few ones with extraordinary popularity.
...
topartist_data@data <- merge(
  topartist_data@data,
  country_all[,
    .(country, allscrobbles, popup = paste(display_values(allscrobbles)))
  ],
  all.x = TRUE,
  by = 'country'
)
...
color_pal <- colorNumeric(
  palette = "plasma",
  reverse = TRUE,
  domain = topartist_data@data$allscrobbles, 
...
topartist_map <- addPolygons(
  ...
  smoothFactor = 0.2, fillOpacity = 0.6, fillColor = ~color_pal(allscrobbles),
