...
```{r missingcountrycheck, echo=TRUE}
...
# genre_country_top <- genre_country[
#   (genre %in% genres[allscrobbles > 10^7 & artists > 80, genre]) &
#     (country %in% country_all[1:25, country]),
# ]
```
...
jazz vs classic rock comparison, summarize.
...
country_bar <- ggplot(
  ...
  coord_flip() +
  labs(
    title = "Where do most listened to music come from?",
    x = "Music origin",
...
For which genres the top producer (by aggregate number of scrobbles) or the top artist is not from USA or UK? 
...
Another interesting question is to observe relative popularity of specific genres within the entire music produced by specific countries

Top 10 genres for top 24 music producers:
TODO withincountry popularity: (% of country-level production)

```{r, relativeproduction}
# todo facets
# [country == 'Poland', ][1:10, ]
test <- ggplot(
  genre_country[order(country_perc, decreasing = TRUE)],
  aes(genre, country_perc)
) +
  geom_col(
    fill = '#ED7953FF'
  ) +
